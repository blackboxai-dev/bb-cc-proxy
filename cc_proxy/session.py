...
class ConfidentialSession:
    ...
    def _wrap_legacy_reply(self, text: str, body: dict) -> dict:
        """Wrap a bare assistant text (legacy worker reply) into an OpenAI
        ``chat.completion`` dict. Extracted so both the envelope path (bare
        text fallback) and the legacy path use the exact same shape.
        """
    ...
    def send_openai(self, body: dict, *, path: str = "/v1/chat/completions",
        ...
        if resp.status_code == 500 and path == "/v1/chat/completions":
            ...
            logger.warning(
                ...
                "wire protocol (tool_calls/usage/finish_reason will be lost) — %s",
                self.model,
            )
            self._wire = "legacy"
            try:
    ...
    def stream_openai(self, body: dict, *, path: str = "/v1/chat/completions",
        ...
        def _post_legacy():
            return self._post_message(body.get("messages") or [], stream=True,
        ...
        if self._wire == "legacy" and path == "/v1/chat/completions":
            ...
            if resp.status_code == 500 and path == "/v1/chat/completions":
                logger.warning(
                    ...
                    "to legacy wire protocol — %s", self.model,
                )
                self._wire = "legacy"
                try:
                    resp.close()
